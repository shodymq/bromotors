...
import { getBrands, getCarsResult } from '../../lib/api';

export const metadata: Metadata = { title: 'Авто в продаже — BRO MOTORS Қызылорда' };
export const dynamic = 'force-dynamic';

export default async function CatalogPage() {
  const [carsResult, brands] = await Promise.all([getCarsResult(), getBrands()]);
  return (
    <>
      <Header />
      <main className="section">
        <div className="container">
          ...
          <CatalogClient cars={carsResult.cars} brands={brands} />
        </div>
      </main>
    </>
  );
